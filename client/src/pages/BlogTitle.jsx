...
const BlogTitle = () => {
  ...
  const blogCategories = [
    'General', 'Technology', 'Nusiness', 'Health', 'Lifestyle', 'education', 'Travel', 'Food'
  ]
  ...
  const onSubmitHandlar = async (e) => {
    ...
    try {
      setLoading(true)
      const prompt = `Generate a blog ttitle for keyword ${input} in the category ${selectedCategory}`
    ...
    <div>
      <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
        ...
        <form onSubmit={onSubmitHandlar} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
          ...
          <input onChange={(e) => setInput(e.target.value)} value={input} type="text" className='w-full p-2 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='The Future of Ai is ...' required />

          <p className='mt-4 test-sm font-medium'>Category</p>
